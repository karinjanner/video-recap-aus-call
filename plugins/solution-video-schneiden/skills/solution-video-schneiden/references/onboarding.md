# Ersteinrichtung einer Solution-Reihe

Ziel ist ein kleines lokales Serienprofil, nicht ein langes Interview. Nur fehlende Entscheidungen erfragen und höchstens drei kurze Fragen auf einmal stellen. Nach jeder Antwort das Profil ergänzen und mit den nächsten offenen Punkten fortfahren.

## Zuerst ermitteln

Diese Angaben können oft schon aus Auftrag, Dateinamen, Folien oder vorhandenen Markenunterlagen belegt werden:

- Name der Solution oder Arbeitsgruppe,
- Thema und Ziel der Reihe,
- Moderator:in oder Gastgeber:in,
- Hauptvortragende der konkreten Sitzung,
- Zielgruppe und gewünschte Nutzung,
- vorhandene Designunterlagen und ausdrücklich für diese Solution bereitgestellte Musikdateien.

Belegte Angaben als Vorschlag zusammenfassen, statt sie erneut offen abzufragen. Namen nie raten.

## Geführte Fragen

### Block 1: Inhalt und Rollen

1. Wie heißt eure Solution und worum geht es in einem Satz?
2. Wer moderiert die Treffen? Gibt es in dieser Aufnahme eine andere Person, die den Hauptvortrag oder die Demo hält?
3. Für wen ist das fertige Video gedacht und was soll diese Person danach verstanden haben?

Ist das Sitzungsthema aus der Aufnahme nicht klar, zusätzlich fragen: „Welches konkrete Thema oder Ergebnis steht in dieser Folge im Mittelpunkt?“

Außerdem klären, ob die vollständige Meeting-Aufzeichnung vorliegt und ob es weitere synchrone Zoom-Perspektiven, Folien, Chat oder Transkriptdateien gibt. Die typische Quelldauer von ungefähr 60–180 Minuten ist kein Grund zum Kürzen vor der Analyse.

### Block 2: Gestaltung

1. Gibt es ein eigenes Design, das verwendet werden soll – etwa Logo, Farben, Schrift, Vorlagen oder ein Referenzvideo?
2. Gibt es gewünschten Text für Titel- und Abschlusskarte?
3. Falls kein Design hochgeladen wird: „Dann baue ich ein Design passend zu euren Folien. Einverstanden?“

**Design aus den Folien ableiten**, wenn kein eigenes Design bereitgestellt wird und die Aufnahme Folien enthält (oder saubere Folien geliefert wurden):

- Aus mehreren typischen Folien die tragenden Farben bestimmen (Hintergrund, Akzent, Text) und auf ausreichenden Kontrast prüfen.
- Den Schriftcharakter übernehmen (zum Beispiel serifenlos und kräftig, oder ruhig mit Serifen) und eine gut lesbare, frei verfügbare Schrift in diesem Charakter wählen. Die Originalschrift nur verwenden, wenn sie lokal vorhanden und frei nutzbar ist.
- Flächen, Ecken, Linien und Abstände aufgreifen, damit Titelkarte, Kapitelkarten, Namenseinblendungen und Abschlusskarte zu den Folien passen.
- Logos, Kunden- oder Partnermarken aus den Folien nicht übernehmen, außer sie werden ausdrücklich für die Videoreihe freigegeben.
- Eine Vorschau von Titelkarte und Namenseinblendung zeigen und bestätigen lassen; das Ergebnis im Serienprofil mit Status „aus Folien abgeleitet“ festhalten.

Gibt es keine Folien, ein ruhiges, barrierearmes neutrales Design verwenden und als vorläufig kennzeichnen.

### Block 3: Musik

1. „Möchtest du für diese Solution Musik verwenden? Wenn ja, welche? Ohne bereitgestellte Musik erstellen wir die Fassung sauber ohne Musik.“
2. Wenn ja: „Bitte lade die Musikdatei hoch oder nenne den eindeutigen lokalen Pfad. Eine Anfangs- und eine Endmusik sind empfehlenswert; das kann derselbe Track oder je ein eigener Track sein. Darf die bereitgestellte Musik für diese Videos verwendet werden?“
3. Separat fragen: „Darf ich geeignete Ausschnitte dieser Musik auch für Themenübergänge innerhalb des Videos verwenden, wenn zeitlich weit auseinanderliegende Statements oder deutliche Gedankensprünge getrennt werden müssen?“

Die Übergangsfreigabe ist unabhängig von der Freigabe für Titel und Abschluss. Ohne eindeutiges Ja keine Musik an Themenübergängen verwenden. Musik unter Sprache ebenfalls nur nach ausdrücklichem Wunsch einsetzen. Ausschließlich Musik verwenden, die für die aktuelle Solution ausdrücklich bereitgestellt und freigegeben wurde. Nie selbständig Musik suchen oder herunterladen und keine Platzhaltermusik verwenden.

### Block 4: Ausgabe

Nur fragen, wenn nicht vorgegeben:

- Format oder Plattform; Standardvorschlag 16:9, 1920 × 1080,
- ob Namen als Lower Third erscheinen dürfen,
- besondere private Inhalte, Personen oder Passagen, die ausgeschlossen werden sollen.

Die Ziellänge beträgt standardmäßig 7–10 Minuten. Nur nach einer anderen Länge fragen, wenn Material, Nutzungszweck oder ausdrücklicher Wunsch eine bewusste Abweichung nahelegt.

## Profil anlegen

Nach der Ersteinrichtung [Serienprofil-Vorlage](series-profile-template.md) als `series-profile.md` in den jeweiligen Aufnahme- oder Projektordner kopieren und mit den bestätigten Angaben ausfüllen. Unbekannte optionale Felder mit `noch offen` markieren; keine Fakten erfinden.

Vor dem Schnitt eine kompakte Zusammenfassung zeigen und ausdrücklich kenntlich machen:

- bestätigt,
- aus dem Material abgeleitet,
- noch offen,
- aus Folien abgeleitet oder vorläufiger neutraler Standard.

Offene Design- oder Musikfragen blockieren die Inhaltsanalyse nicht. Ein unklarer Themenfokus oder unbekannte Hauptrollen blockieren dagegen die endgültige Dramaturgie und müssen vor dem Rendern geklärt werden.
